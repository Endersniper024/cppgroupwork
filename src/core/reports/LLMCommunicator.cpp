...
namespace Core::Reports {
    ...
    LLMCommunicator::LLMCommunicator(QObject *parent) : ILLMCommunicator(parent) {
        ...
        connect(m_networkManager, &QNetworkAccessManager::finished, this, &LLMCommunicator::onReplyFinished);
    }
    LLMCommunicator::~LLMCommunicator() {}


    void LLMCommunicator::configure(const QString& apiKey, const QString& apiUrl, const QString& modelName) {
        m_apiKey = apiKey;
        m_apiUrl = apiUrl;
        m_modelName = modelName; // e.g. "gpt-3.5-turbo"
    }

    QString LLMCommunicator::buildPrompt(const CalculatedMetrics& metrics, const AggregatedData& context) {
        // (This is where the detailed prompt from section 5's example would be constructed dynamically)
        QString prompt = QString(
            "你是一位乐于助人的学习教练。请分析以下学生的学习数据：\n"
            "任务完成率：%1%%\n"
            "总学习时长：%2 分钟。\n"
            "各科目时间分配（百分比）：\n")
        ...
        for (auto it = metrics.timeAllocationBySubjectPercentage.constBegin(); it != metrics.timeAllocationBySubjectPercentage.constEnd(); ++it) {
            prompt += QString("- %1: %2%\n").arg(it.key()).arg(QString::number(it.value(), 'f', 1));
        }
        prompt += "Task Efficiency:\n";
        for (auto it = metrics.taskEfficiencySummary.constBegin(); it != metrics.taskEfficiencySummary.constEnd(); ++it) {
            prompt += QString("- %1: %2\n").arg(it.key()).arg(it.value());
        }
        // Add more context from AggregatedData.relevantTasks or relevantTimeLogs if desired, summarized.
        // For example, number of tasks completed, most time-consuming tasks etc.
        // Make sure not to exceed token limits.
    ...
    void LLMCommunicator::requestInsights(const CalculatedMetrics& metrics, const AggregatedData& contextForLLM, int requestId) {
        ...
        // For Gemini (key in URL or specific header as per their docs, might not use Bearer)
        // If key is in URL, m_apiUrl should already include it during configure()
        // if (m_apiUrl.contains("googleapis.com")) { /* Gemini specific headers if any */ }


        QJsonObject requestBody;
        // Structure depends on the API (OpenAI Chat Completion vs Gemini etc.)
        if (m_apiUrl.contains("openai.com")) { // Example for OpenAI
            requestBody["model"] = m_modelName.isEmpty() ? "gpt-3.5-turbo" : m_modelName;
            QJsonArray messages;
            QJsonObject message;
            message["role"] = "user";
    ...
    void LLMCommunicator::onReplyFinished(QNetworkReply *reply) {
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            insights = parseResponse(responseData, parseSuccess); // Implement this based on LLM's JSON structure
            ...
            QString errorMsg = QString("LLM API Error: %1. HTTP Status: %2. Details: %3")
    ...
    LLMInsights LLMCommunicator::parseResponse(const QByteArray& responseData, bool& success) {
        // This function is highly dependent on the specific LLM API's response format.
        // Example for the JSON structure requested in the prompt:
        // { "summary": "...", "positives": ["..."], "improvements": ["..."], "suggestions": ["..."] }
        LLMInsights insights;
        success = false;
        ...
        } else if (obj.contains("candidates") && obj["candidates"].isArray()) { // Gemini specific parsing
                    ...
                    if (contentObj.contains("parts") && contentObj["parts"].isArray()) {
                        ...
                        if(!parts.isEmpty() && parts.first().isObject() && parts.first().toObject().contains("text")) {
                            ...
                            if (contentDoc.isObject()) {
                                ...
                                qWarning() << "LLM (Gemini) part text is not a JSON object string: " << textContent;
                                insights.summary = textContent;
                                success = !insights.summary.trimmed().isEmpty();
                                return insights;
                            }
                        } else { qWarning() << "LLM (Gemini) content parts missing or invalid."; return insights;}
        ...
        if (obj.contains("positives") && obj["positives"].isArray()) {
            for (const QJsonValue& val : obj["positives"].toArray()) {
                if (val.isString()) insights.positiveObservations.append(val.toString());
            }
        }
        if (obj.contains("improvements") && obj["improvements"].isArray()) {
            for (const QJsonValue& val : obj["improvements"].toArray()) {
                if (val.isString()) insights.areasForImprovement.append(val.toString());
            }
        }
        if (obj.contains("suggestions") && obj["suggestions"].isArray()) {
            for (const QJsonValue& val : obj["suggestions"].toArray()) {
                if (val.isString()) insights.actionableSuggestions.append(val.toString());
            }
        }
        if (!success && !obj.isEmpty()) { // If specific fields not found but got some JSON, try to extract text
            qWarning() << "LLM response JSON did not match expected structure. Trying generic extraction.";
            // Attempt to extract some meaningful text if parsing fails on specific fields
            QString fallbackText;
            for(const QString& key : obj.keys()){
                if(obj[key].isString()) fallbackText += obj[key].toString() + "\n";
                // Could also iterate arrays of strings, etc.
            }
            if(!fallbackText.isEmpty()) {
                insights.summary = tr("LLM Response (unexpected format):\n%1").arg(fallbackText.trimmed());
                success = true;
            }
        }
