...
#include "ProcessLink.h" // 包含程序关联结构体
...
public:
  ...
  // 程序关联管理 (004-关联程序功能)
  bool initProcessLinkTable();
  bool initProcessTimeLogTable();
  bool addProcessLink(ProcessLink &processLink);
  ProcessLink getProcessLinkById(int linkId, int userId);
  QList<ProcessLink> getProcessLinksByTask(int taskId, int userId);
  QList<ProcessLink> getProcessLinksByUser(int userId);
  bool updateProcessLink(const ProcessLink &processLink);
  bool deleteProcessLink(int linkId, int userId);

  // 程序时间记录
  bool addProcessTimeLog(ProcessTimeLog &processTimeLog);
  QList<ProcessTimeLog> getProcessTimeLogsByTask(int taskId, int userId);
  QList<ProcessTimeLog>
  getProcessTimeLogsByUser(int userId, const QDateTime &fromDate = QDateTime(),
                           const QDateTime &toDate = QDateTime());
  qint64 getTotalProcessTimeForTask(int taskId, int userId);
  qint64 getTotalProcessTimeForUser(int userId);
