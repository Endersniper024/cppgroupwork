...
SubjectDialog::SubjectDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::SubjectDialog),
  ...
  // Remove the default connection in .ui file if it connects directly to
  // QDialog::accept connect(ui->buttonBox, &QDialogButtonBox::accepted, this,
  // &SubjectDialog::on_buttonBox_accepted); The .ui connection will call
  // accept() after this slot if it's also connected there. It's better to
  // handle accept() manually here.
}
...
SubjectDialog::SubjectDialog(const Subject &subjectToEdit, QWidget *parent)
  ...
  // &SubjectDialog::on_buttonBox_accepted);
}
...
void SubjectDialog::updateColorPreview() {
  if (m_selectedColor.isValid()) {
    ...
    ui->colorPreviewLabel->setStyleSheet(
...
void SubjectDialog::on_buttonBox_accepted() {
  ...
  if (name.isEmpty()) {
    ...
    // To prevent dialog closing with QDialogButtonBox::accepted() if validation
    // fails, and if the .ui file auto-connects accepted() to QDialog::accept(),
    // this won't stop it. We need to ensure this slot is the only one
    // controlling the accept(). If on_buttonBox_accepted calls
    // QDialog::accept() itself, it's fine.
  ...
  m_subject.color =
      m_selectedColor; // m_selectedColor is updated directly on color change
  m_subject.tags = tags;
  // m_subject.userId and m_subject.id are set externally before calling this
  // dialog (for edit) or userId is set before saving (for new)

  // The actual DB check for subjectExists (name uniqueness for user) should
  // ideally be done *before* calling accept, or the calling code (MainWindow)
  // handles DB interaction and potential errors. For simplicity, this dialog
  // just prepares the data. The caller will handle DB ops.
