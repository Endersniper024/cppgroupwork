...
public:
    ...
    explicit TaskDialog(int currentUserId, const QList<Subject>& subjects, QWidget *parent = nullptr, int defaultSubjectId = -1);
    // Constructor for editing an existing task
    explicit TaskDialog(int currentUserId, const Task& taskToEdit, const QList<Subject>& subjects, QWidget *parent = nullptr);
    ~TaskDialog();
