...
TaskDialog::TaskDialog(int currentUserId, const QList<Subject> &subjects,
                       QWidget *parent, int defaultSubjectId)
    : QDialog(parent), ui(new Ui::TaskDialog), m_currentUserId(currentUserId),
      m_isEditMode(false), m_availableSubjects(subjects) {
  ui->setupUi(this);
  setWindowTitle(tr("创建新任务"));
  m_task.userId =
      m_currentUserId; // Assign current user to the task being created
  ...
  ui->dueDateTimeEdit->setDateTime(
      QDateTime::currentDateTime().addDays(1)); // Default due date tomorrow

  // Ensure validation controls dialog closure
  disconnect(ui->buttonBox, &QDialogButtonBox::accepted, this,
             &QDialog::accept);
  connect(ui->buttonBox, &QDialogButtonBox::accepted, this,
          &TaskDialog::on_buttonBox_accepted);
  connect(ui->buttonBox, &QDialogButtonBox::rejected, this, &QDialog::reject);
}

// Constructor for editing task
TaskDialog::TaskDialog(int currentUserId, const Task &taskToEdit,
                       const QList<Subject> &subjects, QWidget *parent)
    : QDialog(parent), ui(new Ui::TaskDialog),
      ...
      m_isEditMode(true), m_availableSubjects(subjects) {
  ui->setupUi(this);
...
void TaskDialog::populateSubjectComboBox(int defaultSubjectId) {
  ...
  for (int i = 0; i < m_availableSubjects.size(); ++i) {
    ...
    ui->subjectComboBox->addItem(subject.name,
                                 subject.id); // Store subject ID as item data
  ...
  if (selectionIndex != -1) {
    ...
    ui->subjectComboBox->setCurrentIndex(
        0); // Default to first if no match or no default
  }
}

void TaskDialog::populatePriorityComboBox() {
  ui->priorityComboBox->clear();
  ui->priorityComboBox->addItem(priorityToString(TaskPriority::Low),
                                static_cast<int>(TaskPriority::Low));
  ui->priorityComboBox->addItem(priorityToString(TaskPriority::Medium),
                                static_cast<int>(TaskPriority::Medium));
  ui->priorityComboBox->addItem(priorityToString(TaskPriority::High),
...
void TaskDialog::populateStatusComboBox() {
  ...
  ui->statusComboBox->addItem(statusToString(TaskStatus::Pending),
                              static_cast<int>(TaskStatus::Pending));
  ui->statusComboBox->addItem(statusToString(TaskStatus::InProgress),
                              static_cast<int>(TaskStatus::InProgress));
  ui->statusComboBox->addItem(statusToString(TaskStatus::Completed),
...
void TaskDialog::initDialogWithTask(const Task &task) {
  ...
  // Show actual time spent
  ui->actualTimeValueLabel->setText(QString::number(task.actualTimeMinutes));

  // Show program monitoring data
  updateProgramMonitoringInfo(task.id);

  // Actual time and creation/completion dates are not typically edited directly
  // here
}

void TaskDialog::updateProgramMonitoringInfo(int taskId) {
  if (taskId == -1) {
    ui->programTimeValueLabel->setText("0");
    ui->linkedProgramsValueLabel->setText("无");
    return;
  }

  // Get program links for this task
  DatabaseManager &db = DatabaseManager::instance();
  QList<ProcessLink> processLinks =
      db.getProcessLinksByTask(taskId, m_currentUserId);

  // Calculate total program time for this task
  QList<ProcessTimeLog> timeLogs =
      db.getProcessTimeLogsByTask(taskId, m_currentUserId);
  qint64 totalProgramTimeSeconds = 0;

  for (const ProcessTimeLog &log : timeLogs) {
    totalProgramTimeSeconds += log.durationSeconds;
  }

  // Display program time in minutes
  int programTimeMinutes = totalProgramTimeSeconds / 60;
  ui->programTimeValueLabel->setText(QString::number(programTimeMinutes));

  // Display linked programs
  QStringList linkedPrograms;
  for (const ProcessLink &link : processLinks) {
    linkedPrograms.append(link.processName);
  }

  if (linkedPrograms.isEmpty()) {
    ui->linkedProgramsValueLabel->setText("无");
  } else {
    ui->linkedProgramsValueLabel->setText(linkedPrograms.join(", "));
  }
}

Task TaskDialog::getTaskData() const { return m_task; }
...
void TaskDialog::on_buttonBox_accepted() {
  ...
  m_task.priority =
      static_cast<TaskPriority>(ui->priorityComboBox->currentData().toInt());
  m_task.dueDate = ui->dueDateTimeEdit->dateTime();
  m_task.estimatedTimeMinutes = ui->estimatedTimeSpinBox->value();

  TaskStatus newStatus =
      static_cast<TaskStatus>(ui->statusComboBox->currentData().toInt());
  if (m_isEditMode) { // Only allow status change in edit mode, new tasks
                      // default to Pending
    if (m_task.status != newStatus && newStatus == TaskStatus::Completed) {
      m_task.completionDate = QDateTime::currentDateTime();
    } else if (m_task.status == TaskStatus::Completed &&
               newStatus != TaskStatus::Completed) {
      m_task.completionDate =
          QDateTime(); // Clear completion date if moved from completed
    }
    m_task.status = newStatus;
  } else {
    m_task.status = TaskStatus::Pending; // New tasks are always pending
                                         // initially via dialog
  }

  // userId and creationDate are already set in m_task
  // If edit mode, m_task.id is already set.

  qDebug() << "TaskDialog accepted. Task Name:" << m_task.name
           << "SubjID:" << m_task.subjectId;
